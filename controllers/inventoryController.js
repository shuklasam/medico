...
const createInventoryController = async (req, res) => {
  console.log("Calling create inventory");
  try {
    const { email } = req.body;
    // Set organisation from authenticated user
    req.body.organisation = req.userId;
    //validation
    if (email === null) {
    ...
    if (req.body.inventoryType === "in" && user.role !== "donar") {
      return res.status(400).send({
        success: false,
        message: "This email is not registered as a Donor",
      });
    }
    if (req.body.inventoryType === "out" && user.role !== "hospital") {
      return res.status(400).send({
        success: false,
        message: "This email is not registered as a Hospital",
      });
    }
    ...
    let errorMessage = "Error In Create Inventory API";

    // Check for Mongoose specific errors
